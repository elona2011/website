let rectWidth = 70, rectHeight = 70;
...
let data = new RectGroup(), svg = d3.select('svg'), svgWidth = parseInt(svg.style('width')), svgHeight = parseInt(svg.style('height')), index = 10, gMerge, path, pathHover, pathCross, drawLineEnable = false, drawLineFrom, drawLineTo;
...
function ticked() {
    path.attr("d", function (d) {
        let centerX = d.source.x + (d.target.x - d.source.x) / 2, centerY = d.source.y + (d.target.y - d.source.y) / 2;
...
function addEvent() {
    let buttonAdd = document.querySelector('.connect .add'), buttonView = document.querySelector('.connect .view'), buttonEdit = document.querySelector('.connect .edit'), svg = document.querySelector('svg');
...
function mousemove() {
    if (drawLineEnable && drawLineFrom) {
        let p = d3.mouse(this), centerX = drawLineFrom.x / 2 + p[0] / 2, centerY = drawLineFrom.y / 2 + p[1] / 2;
